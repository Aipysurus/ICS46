...
ICS46_DYNAMIC_FACTORY_REGISTER(OthelloAI, kluu5::KevinAI, "KEVIN AI(REQUIRED)");

std::pair<int, int> kluu5::KevinAI::chooseMove(const OthelloGameState& state)
{
	int score;
	int best_score = -9999;
	std::string turn;
	...
	std::vector<std::pair<int,int>> avail_moves = kluu5::KevinAI::generate_moves(othello_copy);
...
int kluu5::KevinAI::search(std::unique_ptr<OthelloGameState>& s, int depth, std::string turn)
{
	std::vector<std::pair<int,int>> potential_moves = kluu5::KevinAI::generate_moves(s);
	int my_score = -9999;
...
int kluu5::KevinAI::evaluate_score(std::unique_ptr<OthelloGameState>& c)
{
	/*int width = c->board().width();
	int height = c->board().height();
	std::vector<std::pair<int,int>> moves = kluu5::KevinAI::generate_moves(c);*/
	if (c->isWhiteTurn())
...
std::vector<std::pair<int,int>> kluu5::KevinAI::generate_moves(std::unique_ptr<OthelloGameState>& s)
{
